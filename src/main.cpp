      ...
      if(end_timer - start_timer <= RTT_THRESHOLD){
        ...
        if((solenoid & 0x04) == 0x04){
          ...
          if(!(xV == 0 && yV == 0 && zV == 0)){
            ...
            imu::Quaternion totalRotation = rot * quat;
            // imu::Quaternion totalRotation = quat;
            imu::Vector<3> XVector(0, 1, 0);
            imu::Vector<3> YVector(1, 0, 0);
            imu::Vector<3> normalXVector = totalRotation.rotateVector(XVector);
            imu::Vector<3> normalYVector = totalRotation.rotateVector(YVector);
            ...
            if(yAngle > xAngle){
              throttleM1 = map(throttle, 0, 100, 90, 180);
              
              
              throttleM3 = map(throttle * tan(xAngle), 0, 100, 90, 180);
              
              throttleM2 = map(throttle, 0, 100, 90, 180);
              throttleM4 = map(throttle, 0, 100, 90, 180);
            }
            byte throttleM1 = yAngle - 90;
            byte throttleM2 = xAngle - 90;
            byte throttleM3 = 90 - xAngle;
            byte throttleM4 = 90 - yAngle;
            // M1.write(xV);
            // M2.write(xAngle - 90);
            M3.write(90 - xAngle);
            M4.write(xV);
